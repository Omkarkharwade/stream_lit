...
def logistics_app():
    ...
    if st.button("Track My Order", key="track_button"):
        if product_name and quantity and address:
            ...
            st.write(f'<div class="track-order">Your order is being shipped to {city}. Expected Delivery Date: {delivery_date}</div>', unsafe_allow_html=True)
