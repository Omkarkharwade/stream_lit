...
def get_product_price(product_name):
    ...
    return st.session_state['products'].get(product_name, random.randint(1000, 50000))
    ...
    if product_name:
        ...
        if st.button("Place Order"):
            ...
            if product_name not in st.session_state['products']:
                st.session_state['products'][product_name] = price
...
def inventory():
    ...
    # Convert the dictionary to DataFrame for display
    df = pd.DataFrame(list(inventory_data.items()), columns=['Product Name', 'Price'])
    st.table(df)
